...
class Cart extends Model
{
    ...
    if ($oldCart) { //if an old cart already exists it will be displayed thru this code
    ...
    if ($this->items) { // overwrite if the item already exists
    if (array_key_exists($product_id, $this->items)) {
    $storedItem = $this->items[$product_id];
    }
    }

    //otherwise put it in the storedd item list in cart database
    $storedItem['qty']++;
